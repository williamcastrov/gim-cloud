...
function RegistraEquipos(props) {
  const { consecutivo, prefijo, idUsu, consecutivoActivo, setPrefijo } = props;
  const history = useHistory();
...
*/
  ...
  const DatosGrupo = (grupo) => {
    let codigo = '"'+grupo+'"'
    //console.log("GRUPO EQUIPO : ", codigo)
   
    async function fetchDataSubGruposEquipos() {
      const res = await subgruposequiposServices.listConsecutivoGrupoSubGrupo(codigo);
